...
public class MapLoader
{
    ...
    /**
     * Méthode de compatibilité - appelle la nouvelle version avec grid = null
     * DEPRECATED: Utilisez loadMap(String path, GridPane grid) à la place
     */
    @Deprecated
    public static Tile[][] loadMap(String path)
    {
        return loadMap(path, null);
    }

    /**
     * Valide qu'une carte chargée est correcte
     *
     * @param tiles La carte à valider
     * @return true si la carte est valide
     */
    public static boolean validateMap(Tile[][] tiles)
    {
        ...
        for (int y = 0; y < tiles.length; y++)
        {
            ...
            for (int x = 0; x < tiles[y].length; x++)
            {
                ...
                else if (tiles[y][x].getType() == Tile.Type.BREAKABLE)
                {
                    hasBreakables = true;
                }
            }
